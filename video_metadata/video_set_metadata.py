...
def set_meta_info(video_file) -> None:
    ...
    video_file = rename_file(
        video_file,
        os.path.join(os.path.dirname(video_file), f"{identifier}_{title_suffix}{extension}"),
    )
    meta_file = rename_file(
        meta_file,
        os.path.join(os.path.dirname(meta_file), f"{identifier}_{title_suffix}_meta.json"),
    )
    meta_file_all = rename_file(
        meta_file_all,
        os.path.join(os.path.dirname(meta_file_all), f"{identifier}_{title_suffix}_meta_all.txt"),
    )
    ...
    if source_image_height:
        commands.append(
            [
                ...
                "largefilesupport=1",
                "-overwrite_original",
                "-xmp:VideoFrameSizeUnit=pixels",
                f"-xmp:VideoFrameSizeH={source_image_height}",
                video_file,
            ]
        )
    if source_image_width:
        commands.append(
            [
                ...
                "-overwrite_original",
                "-xmp:VideoFrameSizeUnit=pixels",
                f"-xmp:VideoFrameSizeW={source_image_width}",
                video_file,
            ]
        )
    if video_frame_rate:
        commands.append(
            [
                ...
                "largefilesupport=1",
                "-overwrite_original",
                f"-xmp:VideoFrameRate={video_frame_rate}",
                video_file,
            ]
        )
    if compressor_name:
        commands.append(
            [
                ...
                "largefilesupport=1",
                "-overwrite_original",
                f"-xmp:VideoCompressor={compressor_name}",
                video_file,
            ]
        )
